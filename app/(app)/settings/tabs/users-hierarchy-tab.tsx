...
export function UsersHierarchyTab({
  ...
  const totalUnits = groupManagers.reduce((sum, gm) => sum + gm.units.length, 0);
  ...
  return (
    <div className="grid grid-cols-[1fr_360px] items-start gap-[22px]">
        ...
        <div className="mt-5 flex flex-col gap-2.5">
          ...
          {groupManagers.map((gm) => (
            <div key={gm.id} className={role === "superadmin" ? "pl-[26px]" : ""}>
              <div className="flex items-center gap-3 rounded-[14px] border border-[#dbeee2] bg-success-bg px-4 py-[13px]">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[11.5px] text-muted">
                    ...
                    {gm.units.reduce((s, u) => s + u.agentCount, 0)} agents
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-2 pl-[26px] pt-2.5">
                {gm.units.map((unit) => (
                  <div
                    key={unit.id}
                    className="flex items-center gap-3 rounded-xl border border-sand-2 bg-cream px-3.5 py-[11px]"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-[11px] text-taupe">
                        {unit.name} · {unit.agentCount} agent{unit.agentCount === 1 ? "" : "s"}
                      </div>
                    </div>
                    {unit.unitManager && (
      ...
      <AddUserForm role={role} assignmentOptions={assignmentOptions} totalUnits={totalUnits} />
    </div>
  );
}
